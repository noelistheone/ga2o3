...
to 100 rows (seed 43, fresh-sample replication), re-join by DOI to the ingest-time verification artifacts that retain
...
Writes results/tier2/quote_audit_alltables_seed43.json."""
import sys, json, re
...
for fom in FOMS:
    ...
    n = min(100, len(pool))
    samp = pool.sample(n=n, random_state=43)
...
out["_method"] = ("per-FOM sample of <=100 DOI-bearing rows (seed 43, fresh-sample replication); quotes recovered from the "
...
(PROJ / "results/tier2/quote_audit_alltables_seed43.json").write_text(json.dumps(out, indent=1))
print("wrote results/tier2/quote_audit_alltables_seed43.json")
