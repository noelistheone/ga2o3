"""Build Fe_Ga configuration-coordinate relax inputs (q=0, q=-1) — the E2 DLTS deep-acceptor
trap (Ec−0.86 eV, Neal 2018). Fe_Ga NMP capture was computed by Suo et al. 2022, so this defect
gives a VALIDATION anchor for the sandbox's CC→NMP→τ pipeline. Fe substitutes one Ga."""
import re
import warnings
...
CC = PROJ / "dft/qe_cc"
MASS = {"Ga": 69.723, "O": 15.999, "Fe": 55.845}
UPF = {"Ga": "Ga.upf", "O": "O.upf", "Fe": "Fe.upf"}
...
sub = ga[int(np.argmin([np.linalg.norm(sc[i].coords - center) for i in ga]))]
sc.replace(sub, "Fe")
comp = {}
for s in sc:
    comp[s.specie.symbol] = comp.get(s.specie.symbol, 0) + 1
print(f"Fe_Ga supercell: {len(sc)} atoms {comp}")
...
for q in (0, -1):
    ...
    h = re.sub(r"starting_magnetization\(1\) = [\d.]+", "starting_magnetization(1) = 0.3", h)
