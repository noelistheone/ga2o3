"""Phase 51 — Single-task encoder pretrain on Witman/Goyal V_O f.e.

Trains a *separate* CGCNN encoder from scratch (or warm-started from
v2 checkpoint) to predict the V_O formation enthalpy across 199 oxide
hosts (795 total V_O entries, Witman et al. Nat Comp Sci 2023).

Output: `checkpoints/pretrained_encoder_witman.pt` — a frozen feature
extractor whose 64-dim output the Phase 51 MultiEncoderFusion will
fuse alongside MP and JARVIS encoders via self-attention.

This is a NEW file — does not touch existing pretrain code.

Usage:
    conda activate ga2o3
    python scripts/02c_pretrain_witman.py --gpu 0 --epochs 200
"""
...
def main():
    ...
    p.add_argument("--warm-start",
                   ...
                   help="MP-pretrained encoder to warm-start from. "
                        "Set to '' for from-scratch.")
    p.add_argument("--out", default="checkpoints/pretrained_encoder_witman.pt")
    ...
    # ---- Dataset ----
    ds = WitmanVacancyDataset(
        master_csv=str(PROJ / args.master_csv),
        cache_dir=str(PROJ / args.cache_dir),
        normalize=True,
    )
    n_total = len(ds)
    n_val = max(20, int(n_total * args.val_frac))
    n_train = n_total - n_val
    train_ds, val_ds = random_split(
        ds, [n_train, n_val],
        generator=torch.Generator().manual_seed(42),
    )
    logger.info(f"Witman: {n_total} samples → {n_train} train / {n_val} val")
    ...
    encoder = CGCNNEncoder(
        ...
        pretrain=True, num_targets=1,   # single target: V_O f.e.
    )
    if args.warm_start and (PROJ / args.warm_start).exists():
        ...
        # Drop pretrain_head (different shape — MP head is 2-d, ours is 1-d)
